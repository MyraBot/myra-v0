...
public class EventWaiter {
    ...
    public void buy(GuildMessageReceivedEvent event) {
        ...
        if (message.equalsIgnoreCase("yes") || message.equalsIgnoreCase("y")) { // Check if message is 'yes' or 'y'
            ...
            final GetMember memberDb = new Database(event.getGuild()).getMembers().getMember(event.getMember()); // Get member in database

            // Maximum amount of money would be reached
            if (memberDb.getInteger("balance") + role.getPrice() / 2 > Config.ECONOMY_MAX) {
                final int sellingPrice = Config.ECONOMY_MAX - memberDb.getInteger("balance"); // Get money to add until you reach the maximum

                EmbedBuilder sell = new EmbedBuilder()
                        .setAuthor("buy", null, event.getAuthor().getEffectiveAvatarUrl())
                        .setColor(Utilities.getUtils().blue)
                        .setDescription("Sold " + event.getGuild().getRoleById(role.getId()).getAsMention() + " for " + sellingPrice + new Database(event.getGuild()).getNested("economy").getString("currency"));
                event.getChannel().sendMessage(sell.build()).queue(); // Send success message
                memberDb.setBalance(Config.ECONOMY_MAX); // Set balance to the maximum
            }
